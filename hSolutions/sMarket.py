...
def cal_market_return(bgn_date: np.int32, end_date: np.int32, available_dir: str, sectors: list[str]) -> pd.DataFrame:
...
def main_market(
    ...
    if ss.check_daily_continuous(bgn_date, calendar) == 0:
        ret_by_sector = cal_market_return(bgn_date, end_date, available_dir, sectors=sectors)
